...
def test_get_instant_check_content_xml_is_well_formed():
    ...
    try:
        parser.parse('http://10.0.0.47/cgi-bin/egauge?inst&tot')
    except Exception:
        assert False
...
def test_get_instant_check_xml_syntax_is_valid():
    try:
        doc = etree.parse('http://10.0.0.47/cgi-bin/egauge?inst&tot')
...
def test_get_history_check_content_xml_is_well_formed():
        ...
        parser.setContentHandler(ContentHandler())
        try:
            parser.parse('http://10.0.0.47/cgi-bin/egauge-show')
        except Exception:
            assert False
...
def test_get_history_check_xml_syntax_is_valid():
        try:
            doc = etree.parse('http://10.0.0.47/cgi-bin/egauge-show')
        except etree.XMLSyntaxError:
            assert False
